            ...
            <CardContent className="flex flex-col gap-4">
              ...
              {notes.length === 0 ? (
                ...
                <ul className="flex flex-col gap-3">
                  {notes.map((note) => (
                    <li key={note.id} className="rounded-md border border-slate-100 bg-slate-50 p-3">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                  {photos.map((photo) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="group flex flex-col gap-1 overflow-hidden rounded-md border border-slate-200"
                    >
                      ...
                      <div className="px-2 pb-2">
                        ...
                        {photo.description && <p className="mt-1 truncate text-xs text-slate-500">{photo.description}</p>}
                        <p className="text-[11px] text-slate-400">{formatDateUK(photo.created_at)}</p>
                      </div>
                    </a>
                  ))}
                </div>
                ...
                <ul className="divide-y divide-slate-100">
                  {documents.map((doc) => (
                    <li key={doc.id} className="flex items-center justify-between py-2.5">
                      <div>
                        {doc.signed_url ? (
                          <a href={doc.signed_url} target="_blank" rel="noreferrer" className="text-sm font-medium text-slate-900 hover:underline">
                            {doc.filename}
                          </a>
                        ) : (
                ...
                <ul className="divide-y divide-slate-100">
                  {timesheets.map((t) => (
                    <li key={t.id} className="flex items-center justify-between py-2.5">
                      <div>
                        <p className="text-sm font-medium text-slate-900">{t.profile?.full_name ?? "—"}</p>
                        <p className="text-xs text-slate-500">
                      ...
                      {costs.map((c) => (
                        <li key={c.id} className="flex items-center justify-between py-2.5">
                          ...
                          <div className="text-right">
                            ...
                            <p className="mt-1 text-sm font-medium text-slate-900">{formatCurrencyGBP(c.total)}</p>
                          </div>
                        </li>
                      ))}
                    </ul>
                    <div className="flex justify-end gap-6 border-t border-slate-100 pt-3 text-sm">
                      <span className="text-slate-500">VAT: {formatCurrencyGBP(costsVatTotal)}</span>
                      <span className="font-semibold text-slate-900">Total: {formatCurrencyGBP(costsTotal)}</span>
                    </div>
                  </div>
                )
              ) : (
